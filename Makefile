...
CFLAGS      :=  -O3 -fopenmp -pedantic -Wall -Wextra -Wundef -Wshadow -Wpointer-arith -Wcast-align -Wstrict-prototypes -Wwrite-strings -Wcast-qual -Wswitch-default -Wunreachable-code -Wformat=2 -Winit-self -march=native -g -std=gnu99 
LIB         := -fopenmp -lm -lfreeimage -lhdf5 -lcfitsio -lgsl -lgslcblas  -lfftw3f_omp -lfftw3f  -L/usr/local/hdf5/lib/
INC         := -I/usr/local/include  -I/usr/include -I/usr/local/include -I/usr/local/hdf5/include
INCDEP      := 

#---------------------------------------------------------------------------------
#DO NOT EDIT BELOW THIS LINE
#---------------------------------------------------------------------------------
SOURCESA     := $(shell find $(SRCDIR) -type f \( -name *.$(SRCEXT) ! -name "*step*.c" ! -name "moschini*" \))
...
misc: polyImage check bubbles kde rats
#Remake
remake: cleaner all
