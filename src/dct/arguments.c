...
void print_args(Arguments *args, ulong* dims) {
  ...
  args->infile_arg == 0 ?
    info("Input image:  %s.%s", args->inprefix_arg, args->intype_arg):
    info("Input images:  %s-[X].%s", args->inprefix_arg, args->intype_arg);

  if(args->saveout_arg)
    args->outfile_arg == 0 ?
      info("Output image: %s.%s", args->outprefix_arg, args->outtype_arg):
      info("Output images: %s-[X].%s", args->outprefix_arg, args->outtype_arg);
  else
    info("No output files");
  
  info("Tile (process 0): size %ld pixels (width %ld, height %ld, depth %ld)",  dims[0]*dims[1]*dims[2], dims[0], dims[1], dims[2]);
  info("Number of processes: %d (MPI: %d, threads: %d)", np()*args->threads_arg, np(), args->threads_arg);
  info("Grid division: %d x %d x %d", 	args->grid_arg[0], args->grid_arg[1],  args->grid_arg[2]);
  info("Bit depth: %d bit per pixel (%s)", args->bpp_arg, args->refine_arg ? "Data was transformed internally" : "Original dataset");
  info("Type of tree: %s-tree",  	args->tree_arg);
  info("Morphological operation: %s",   args->morphology_arg);
  info("Attribute choice: %s",     	AttribsArray[args->attribute_arg].name);
  info("Pruning decision choice: %s",   Decisions[args->decision_arg].name);
  info("Type of operation: %s",      	args->filter_arg);
  if(args->flood_arg == 2)        info("Flooding algorithm: Wilkinson approach");
  else if(args->flood_arg == 1)   info("Flooding algorithm: Improved Wilkinson approach");
  else  	                 info("Flooding algorithm: Salembier approach");
  info("Connectivity: %d neighbours",   args->connectivity_arg);
  info("Lambda: %0.2lf",       		args->lambda_arg);
  info("lambda vector file: %s", 	args->lvec_arg);
  info("Scaling of the lambda vector values: %0.2lf", 	args->imscale_arg);
  info("Including background intensity in pattern spectra: %d", args->background_arg);
  info("Verbosity: %s",     		args->verbosity_arg);
  // info("Interactive mode: %d",     	args->interactive_arg);
  // info("Save local tree: %d",     	args->savetree_arg);
  info("*** END OF ARGUMENTS PRINTING, STARTING ***\n");
} /* print_args */
