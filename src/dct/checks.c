...
ulong *attribute_offsets(Arguments *args, ulong dims[3]){
  ...
  for (int i = 3; i-- ; ) {
    ...
    debug("%ld, %ld, %ld", attr_off[0], attr_off[1], attr_off[2]);
...
void check_operation(Arguments *args, Node *tree, value offset){
 if ((!strcmp(args->tree_arg, "min") && !strcmp(args->morphology_arg, "opening")) ||
    ...
    for (ulong i = 0; i < tree->size; i++) 
      tree->gval[i] = ((value) g_max_greyval) - tree->gval[i];    
  }
}
...
void check_boundary(Boundary *b) {
  printf("Proc %d, Boundary exists of array %p, dims %zu %zu %zu, offset %zu %zu %zu %zu %zu %zu %zu, size %zu \n", rank(), (void *)  b->array, b->dims[0], b->dims[1], b->dims[2],
   ...
   for (ulong a = 0; a < b->size_curr; a++) {
    printf("%zu: index %ld gval %f border_idx %ld border_par %ld \n",
	   a, (b->array[a]).index, (float) (b->array[a]).gval, (b->array[a]).border_idx, b->border_par[a].i);
   }
} /* check_boundary */
  ...
  for (ulong a = 0; a < size; a++) {
     ...
     if(maxtree->attribute[a]) {
      ...
      attribb[5] = thisdata->maxZ;*/
       attrib = (*attribute)(maxtree->attribute[a]);
      }
    // else intertia = 0;
    printf("index %ld,  parent %ld, gval %f, attribute %lf \n",
	   a, maxtree->parent[a],  (float) maxtree->gval[a],  attrib );
  }
} /* check_maxtree */
