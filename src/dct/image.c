...
value *read_input(Arguments *args, char *prefix, ulong dims[3], ulong dims_T[3], bool *border){
  ...
  if(!strcmp(args->image_arg, "classic")){
    if (args->infile_arg == 0)
      asprintf(&fname, "%s.%s", prefix, args->intype_arg);
    else if (args->infile_arg == 1)
      asprintf(&fname, "%s-%d.%s", prefix, tile, args->intype_arg);
    

    if (args->infile_arg == 0 || (args->infile_arg  && args->overlap_arg)){
      if (!strcmp(args->intype_arg, "h5")) 
	img = read_hdf5(args, fname, dims, dims_T,  &bitpix);     
      else if (!strcmp(args->intype_arg, "fits") || !strcmp(args->intype_arg, "fit"))     
	img = read_fits(args, fname, dims, dims_T, &bitpix);
      else if (!strcmp(args->intype_arg, "nii"))
	img = read_nifti_file(args, fname, dims, dims_T, &bitpix);
      else if (!strcmp(args->intype_arg, "raw"))
	img = read_raw_file(args, prefix, dims, dims_T, &bitpix);
      else  
	img = read_basic(args, fname, dims, dims_T, &bitpix);
    } else {
      if (!strcmp(args->intype_arg, "h5")) 
	img_curr = read_hdf5(args, fname, dims, dims_T,  &bitpix);     
      else if (!strcmp(args->intype_arg, "fits") || !strcmp(args->intype_arg, "fit"))     
	img_curr = read_fits(args, fname, dims, dims_T, &bitpix);
      else  
	img_curr = read_basic(args, fname, dims, dims_T, &bitpix);
    }
 
    if (args->bpp_orig == NULL){
      args->bpp_arg = bitpix;
    }else{
      if (bitpix != args->bpp_arg && myrank == 0)
    ...
    debug("Combining RGB channels");
  
    if(!FLOAT_TYPE)
      warn("Floating point values not activated !!!!!!!!!! ");
  ...
  if(args->infile_arg){
    ...
    if (!args->overlap_arg){
      ...
      img = calloc(new_dims[0]*new_dims[1]*new_dims[2], sizeof(value));
...
value *read_fits(Arguments *args, const char *fname, ulong dims[3], ulong dims_T[3], int *bitpix){
  ...
  if(FLOAT_TYPE)
    ...
    type = (int) (log2(sizeof(value))+1)*10 + negative;
  ...
  if (!fits_open_file(&fptr, fname, READONLY, &status)) {	/* Open file */
	...
	img  =  malloc(dims[0]*dims[1]*dims[2] * sizeof(value));  check_alloc(img, 702);
...
value *read_hdf5(Arguments *args, const char *fname, ulong dims[3], ulong dims_T[3], int *bitpix){
  ...
  debug("Reading HDF5 file %s", fname);
  ...
  value *img = malloc(counts[0]*counts[1]*counts[2] * sizeof(value));   check_alloc(img,710);
  ...
  if(FLOAT_TYPE)
    ...
    err = negative ? H5Dread(dataset_id, H5T_NATIVE_UCHAR, memory_window, dataspace, H5P_DEFAULT, img):
...
value *read_basic(Arguments *args, const char *fname, ulong dims[3], ulong dims_T[3],  int *bitpix){
  ...
  debug("Reading image %s with FreeImage", fname);
  ...
  *bitpix = FreeImage_GetBPP(dib);				/* Get dynamic range */
  ...
  value *img = malloc(dims[0]*dims[1] * sizeof(value));	check_alloc(img,718);
  i = 0;
  if (*bitpix <= 8) {
...
value *read_nifti_file(Arguments *args, const char *fname, ulong dims[3], ulong dims_T[3],  int *bitpix){
  ...
  *bitpix = hdr.bitpix;
  ...
  fp = fopen(fname,"r");
  if (fp == NULL) {
    error(stderr, "Error opening data file %s",fname);
    exit(1);
  }

  ret = fseek(fp, (long)(hdr.vox_offset), SEEK_SET);
  if (ret != 0) {
    error(stderr, "Error doing fseek() to %ld in data file %s",(long)(hdr.vox_offset), fname);
    exit(1);
  }


/********** allocate buffer and read first 3D volume from data file */
  data = (value *) malloc(sizeof(value) * hdr.dim[1]*hdr.dim[2]*hdr.dim[3]);
  if (data == NULL) {
  ...
  if(*bitpix < 0)
    ...
    ret = fread(data, sizeof(ubyte), hdr.dim[1]*hdr.dim[2]*hdr.dim[3], fp);
  else if(*bitpix <=16)
    ret = fread(data, sizeof(ushort), hdr.dim[1]*hdr.dim[2]*hdr.dim[3], fp);
  else if(*bitpix <=32)
    ret = fread(data, sizeof(uint), hdr.dim[1]*hdr.dim[2]*hdr.dim[3], fp);
...
value *read_raw_file(Arguments *args, const char *fname, ulong dims[3], ulong dims_T[3],  int *bitpix){
  ...
  int 		negative      = args->negative_arg;
  int 		infile        = args->infile_arg;
  ulong 	counts[3]     = {1,1,1};
  
  dims_T[0] = args->dims_arg[0];
  dims_T[1] = args->dims_arg[1];
  dims_T[2] = args->dims_arg[2];
  ...
  debug("RAW: tile dimensions: depth %ld, height %ld, width %ld.\n First pixel offset: depth %ld, height %ld, width %ld", dims[0], dims[1], dims[2], dims_T[0], dims_T[1], dims_T[2]);
  ulong size = dims[0]*dims[1]*dims[2];
  value *data = calloc(size, sizeof(value));

  if( args->bpp_arg == -64){
    for(ulong i = 0; i < dims[2]; i++){
      for(ulong j = 0; j < dims[1]; j++){
	for(ulong k = 0; k < dims[0]; k++){
	  if (k==0)
	    fseek(fp, ((i+offsets[2])*(dims_T[0]*dims_T[1])+(offsets[1]+j)*dims_T[0]+k+offsets[0])*sizeof(double),SEEK_SET);
	ulong index = i*dims[0]*dims[1]+j*dims[0]+k;
	double val;
  ...
  } else if (args->bpp_arg == -32){
    for(ulong i = 0; i < dims[2]; i++){
      for(ulong j = 0; j < dims[1]; j++){
	for(ulong k = 0; k < dims[0]; k++){
	  if (k==0)
	    fseek(fp, ((i+offsets[2])*(dims_T[0]*dims_T[1])+(offsets[1]+j)*dims_T[0]+k+offsets[0])*sizeof(float),SEEK_SET);
	  ulong index = i*dims[0]*dims[1]+j*dims[0]+k;
	  float val;
  ...
  } else if (args->bpp_arg == 8){
    for(ulong i = 0; i < dims[2]; i++){
      for(ulong j = 0; j < dims[1]; j++){
	for(ulong k = 0; k < dims[0]; k++){
	  if (k==0)
	    fseek(fp, ((i+offsets[2])*(dims_T[0]*dims_T[1])+(offsets[1]+j)*dims_T[0]+k+offsets[0])*sizeof(char),SEEK_SET);
	  ulong index = i*dims[0]*dims[1]+j*dims[0]+k;
	  char val;
	  fread(&val, sizeof(char), 1, fp+offsets[0]);
	  data[index] =(value) val;
	}
      }
    }
  } else if (args->bpp_arg == 16){
    for(ulong i = 0; i < dims[2]; i++){
      for(ulong j = 0; j < dims[1]; j++){
	for(ulong k = 0; k < dims[0]; k++){
	  if (k==0)
	    fseek(fp, ((i+offsets[2])*(dims_T[0]*dims_T[1])+(offsets[1]+j)*dims_T[0]+k+offsets[0])*sizeof(short),SEEK_SET);
	  ulong index = i*dims[0]*dims[1]+j*dims[0]+k;
	  short val;
	  fread(&val, sizeof(short), 1, fp+offsets[0]);
	  data[index] =(value) val;
	}
      }
    }
	...
	for(ulong k = 0; k < dims[0]; k++){
	  if (k==0)
	    fseek(fp, ((i+offsets[2])*(dims_T[0]*dims_T[1])+(offsets[1]+j)*dims_T[0]+k+offsets[0])*sizeof(int),SEEK_SET);
	  ulong index = i*dims[0]*dims[1]+j*dims[0]+k;
	  int val;
	  fread(&val, sizeof(int), 1, fp+offsets[0]);
	  data[index] =(value) val;
	}
      }
    }
  } else if (args->bpp_arg == 64){
    for(ulong i = 0; i < dims[2]; i++){
      for(ulong j = 0; j < dims[1]; j++){
	for(ulong k = 0; k < dims[0]; k++){
	  if (k==0)
	  ...
	  ulong index = i*dims[0]*dims[1]+j*dims[0]+k;
	  long val;
	  fread(&val, sizeof(long), 1, fp+offsets[0]);
	  data[index] =(value) val;
	} 
      }
    }
...
void write_output(Arguments *args, value *img, const char *attr_name, ulong dims_T[3], ulong dims[3],  bool border[6], int bitpix){
  ...
  if (args->outfile_arg == 0)
    asprintf(&fname_out, "%s_l%.1lf.%s", args->outprefix_arg, args->lambda_arg,args->outtype_arg);
  else{
    asprintf(&fname_out, "%s_l%.1lf-%d.%s", args->outprefix_arg,  args->lambda_arg, tile,  args->outtype_arg);
    if(rank() == 0)
  ...
  if (!strcmp(args->outtype_arg, "h5")){
    ...
    write_hdf5(args, fname_out, dataset_out, img, dims_T, dims, border, bitpix);
  }  else if (!strcmp(args->outtype_arg, "fits") || !strcmp(args->outtype_arg, "fit"))     
    write_fits(args, fname_out, img, dims_T, dims, border, bitpix);      
  else 
    write_basic(args, fname_out, img, dims_T, dims, border, bitpix);   
...
void write_differential(Arguments *args, value *outOrig, value *outDH, value *outScale, const char *attr_name, ulong dims_T[3], ulong dims[3],  bool border[6], int bitpix) {
  ...
  for(int i = 0; i<3; i++ ){
    ...
    if (!strcmp(args->outtype_arg, "h5")) {
      ...
      write_hdf5(args, fname_out, &dataset_out, out,  dims_T, dims, border, bitpix);     
    } else if (!strcmp(args->outtype_arg, "fits") || !strcmp(args->outtype_arg, "fit"))
      write_fits(args, fname_out, out,  dims_T, dims, border, bitpix);
    else 
      write_basic(args, fname_out, out, dims_T, dims, border, bitpix);   
...
void write_basic(Arguments *args, const char* fname, value *out, ulong dims_T[3], ulong dims[3],  bool border[6], int bitpix){
  ...
  if(dims[2] > 1 || (dims_T[2] > 1 && !outfile)){
    ...
    write_fits(args, fname, out, dims_T, dims, border, bitpix);
    return;
  }
  if(!outfile && np() > 1){
    if(rank() == 0)error("FreeImage does not handle writing 3D images, using FITS");
