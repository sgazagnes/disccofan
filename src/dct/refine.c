...
void apply_refine(value *gvals, ulong size, float *factors){
  ...
  for (ulong i = 0; i < size; i++) 
   ...
   float *extrema_all = find_extremas(gvals, size);
   MPI_Allreduce(extrema_all, extrema_all, 1, MPI_FLOAT, MPI_MIN, MPI_COMM_WORLD);
   debug("New minimum value: %f", extrema_all[0]);
   MPI_Allreduce(extrema_all+1, extrema_all+1, 1, MPI_FLOAT, MPI_MAX, MPI_COMM_WORLD);
   debug("New maximum value: %f", extrema_all[1]);
}
