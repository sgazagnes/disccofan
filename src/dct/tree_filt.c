...
void tree_filtering(Node *tree, value *out, ulong size_tile,  int decision_choice, int attrib_choice, double lambda){
  ...
  bool   *reached    = calloc(tree->size, sizeof(bool)); check_alloc(reached, 600);
...
void tree_filter_min(Node *tree, value *out, bool *reached, ulong lwb, ulong upb, double (*attribute)(void *), double lambda) {
  ...
  for (v = lwb; v < upb; v++) {
	...
	if ((tree->attribute[w]) && (*attribute)(tree->attribute[w]) < lambda)  r=w;
        w = parent;
        parent = get_parent(tree, w);
      }
      ...
      if (reached[w] && (out[w] != tree->gval[w])) {
        ...
        val = out[w];
	r = w;
      } else if ((tree->attribute[r]) &&  (*attribute)(tree->attribute[r]) >= lambda) {
        /* w satisfies criterion */
        val = tree->gval[r]; 
      } else if (((!tree->attribute[r]) || ((*attribute)(tree->attribute[r]) < lambda)) && (tree->parent[r] != BOTTOM)) {
...
void tree_filter_direct(Node *tree, value *out, bool *reached, ulong lwb, ulong upb, double (*attribute)(void *), double lambda) {
  ...
  for (v = lwb; v < upb; v++) {
    if (!reached[v]) { /* not filtered yet */
      ...
      /* repeat while we're not at the bottom, th */
      while ((parent != BOTTOM) && (!reached[w]) && ((tree->gval[w] == tree->gval[parent] )  ||  (!tree->attribute[w] || (*attribute)(tree->attribute[w]) < lambda))) {
        w = parent;
        parent = tree->parent[w];	      
      }
...
void tree_filter_subtractive(Node *tree, value *out, bool *reached, ulong lwb, ulong upb, double (*attribute)(void *), double lambda) {
  ...
  for (v = lwb; v <  upb; v++) {
	...
	u = levpath[j];
	if (w != BOTTOM) {
	  if (!tree->attribute[u] || ((*attribute)(tree->attribute[u])) < lambda)
	    filter[j] = filter[j+1];
	  else
	    filter[j] = tree->gval[u] + filter[j+1] - tree->gval[w];
	} else if (tree->attribute[u] && ((*attribute)(tree->attribute[u])) >= lambda)
...
void node_differential_seg(Node *tree, LambdaVec *lvec, value *out_dh, value *temp_dh, value *out_orig, value *out_scale, value *temp_scale, bool *temp_valid, ulong lwb, ulong upb, ulong current,  value *maxDH, value *curDH, value *maxOrig,  int *maxScale, int *curScale,  double (*attribute)(void *)){
  ...
  int scale, numscales = lvec->num_lambdas; 
  idx parent;
  value DH;

  if (is_levelroot(tree, current) && tree->attribute[current]){

    scale = find_scale(lvec, (*attribute)(tree->attribute[current]));
    DH = ((tree->parent[current] != BOTTOM) && (scale < numscales)) ?
      tree->gval[current] - tree->gval[tree->parent[current]] : 0;

  }
  if (is_levelroot(tree, current) && tree->attribute[current] && ((scale == numscales) || (tree->parent[current] == BOTTOM))){
...
void tree_pattern_spectrum(Node *tree,  ulong size, LambdaVec *lvec, double *copy_attr, value *gvals_par, double* spectrum, int background, double (*attribute)(void *)){
  ...
  #pragma omp parallel
  {
	...
	if ((copy_attr[v] != -DBL_MAX)  && (tree->parent[v] != BOTTOM || background)) {
	  scale = find_scale(lvec, (*attribute)(tree->attribute[get_levelroot(tree, v)]));
	  if (scale < numscales) {
	    ...
	    while ((tree->parent[parent] != BOTTOM || background) && (gvals_par[v] < tree->gval[parent])) {    
	      u = parent;
	      scale = find_scale(lvec, (*attribute)(tree->attribute[u]));
	      if (scale>=numscales)
		break;
	...
	if(is_levelroot(tree, v) && (tree->parent[v] != BOTTOM || background)){
	  ...
	  if (dh) {
	    scale = find_scale(lvec, (*attribute)(tree->attribute[v]));
	    if (scale < numscales) 
	      spectrum_th[(numscales)*id + scale] += dh * (*attribute)(tree->attribute[v]);	
	  }
	}
      }
    }
