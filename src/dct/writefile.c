...
void write_inertiaeor_file_binary(FILE *fp,  void *inertiaattr) {

  InertiaDataEoR *inertiadata = inertiaattr;
  fprintf(fp, "%lf ",  inertiadata->area);
  fprintf(fp, "%lf ",  inertiadata->sumX);
  fprintf(fp, "%lf ", inertiadata->sumY);
  fprintf(fp, "%lf ", inertiadata->sumZ);
  fprintf(fp, "%lf ", inertiadata->sumX2);
  fprintf(fp, "%lf ", inertiadata->sumY2);
  fprintf(fp, "%lf ", inertiadata->sumZ2);
  fprintf(fp, "%lf ", inertiadata->sumXZ);
  fprintf(fp, "%lf ", inertiadata->sumYZ);
  fprintf(fp, "%lf ", inertiadata->sumXZ);

} /* write_inertia_file_binary */


void *read_inertiaeor_file_binary(AuxDataStore *store, FILE *fp_attr){
  ulong *initval = malloc(5 * sizeof(ulong));
  void *attr = NULL;	
  fscanf(fp_attr, "%lu ", initval);
  fscanf(fp_attr, "%lu ", initval + 1);
  ...
  attr = load_inertia_data(store, initval);
  free(initval);
  return attr;
}
