...
void init_extent_data(void *rectattr, bool init, ulong x, ulong y, ulong z, value gval){
  ...
  if(init){
    extentdata->minX = extentdata->maxX = x;
    extentdata->minY = extentdata->maxY = y;
    extentdata->minZ = extentdata->maxZ = z;
  } else {
...
double extent_x_attribute(void *rectattr){
  ...
  ExtentData *extentdata = rectattr;
  return (double) ( MAX(pixdim[0]*(extentdata->maxX - extentdata->minX),pixdim[1]*(extentdata->maxY - extentdata->minY)));
...
void *new_morphology_data(AuxDataStore *store){
   MorphologyData *morphdata;
   morphdata = store ? get_new_aux_data(store) :  calloc(1, sizeof(MorphologyData));
   check_alloc(morphdata, 102);
   return(morphdata);
} /* new_morphology_data */

void init_morphology_data(void *morphattr, bool init, ulong x, ulong y, ulong z, value gval){
  MorphologyData *morphdata = morphattr;
  morphdata->area = (ulong) init;
  morphdata->maxval = (double) gval;
  morphdata->sumval = (double) gval;
}


void delete_morphology_data(void *morphattr){
  free(morphattr);
} /* delete_morphology_data */


void add_to_morphology_data(void *morphattr, void *data){
   MorphologyData *morphdata = morphattr;
   morphdata->area++;
} /* add_to_morphology_data */

void merge_morphology_data(void *morphattr, void *childattr){
   MorphologyData *morphdata = morphattr;
   MorphologyData *childdata = childattr;
   morphdata->area += childdata->area;
   morphdata->maxval = MAX(morphdata->maxval, childdata->maxval);
   morphdata->sumval += childdata->sumval;
} /* merge_morphology_data */

void merge_to_morphology_data( AuxDataStore *store, void **thisattr, void *morphattr, void *childattr){
  MorphologyData *thisdata = *thisattr;
  MorphologyData *morphdata = morphattr;
  MorphologyData *childdata = childattr;

  if (!thisdata) {
    thisdata = store ? get_new_aux_data(store) : calloc(1, sizeof(MorphologyData));
    check_alloc(thisdata, 104);
    *thisattr = thisdata;
  }
  thisdata->area = morphdata->area + childdata->area;
  thisdata->maxval = MAX(morphdata->maxval, childdata->maxval);
  thisdata->sumval =  childdata->sumval+morphdata->sumval;
} /* merge_to_morphology_data */

void clone_morphology_data( AuxDataStore *store, void **thisattr, void *morphattr){
  ...
  thisdata->area = morphdata->area;
  thisdata->maxval = morphdata->maxval;
  thisdata->sumval = morphdata->sumval;
} /* clone_morphology_data */


double morphology_area_attribute(void *morphattr){
   MorphologyData *morphdata = morphattr;
   double area;
   area = pixdim[0]*pixdim[1]*pixdim[2]*morphdata->area;
   return( area);
} /* morphology_attribute */

double morphology_attribute(void *morphattr){
   MorphologyData *morphdata = morphattr;
   double area;
   //   if(area>100){
   // printf("%f \n",morphdata->sumval);
   //}
   area = pixdim[0]*pixdim[1]*pixdim[2]*morphdata->area;
   return( morphdata->maxval< 0.02 && area > 3.14*6*6? morphdata->maxval/(morphdata->sumval/18.11):0);
   //return morphdata->maxval*sqrt(3.14*6);
} /* morphology_attribute */


double morphology_fmax_attribute(void *morphattr){
   MorphologyData *morphdata = morphattr;
   double area;
   //   if(area>100){
   // printf("%f \n",morphdata->sumval);
...
AttribStruct AttribsArray[NUMATTR] =
{
 ...
 {"Non-compactnesss",sizeof(InertiaData), new_inertia_data, init_inertia_data, delete_inertia_data, add_to_inertia_data, merge_inertia_data, merge_to_inertia_data, clone_inertia_data, create_mpi_inertia_type, inertia_area_attribute, inertia_ncom_attribute}, //17
  {"Core Prominence",sizeof(MorphologyData), new_morphology_data, init_morphology_data, delete_morphology_data, add_to_morphology_data, merge_morphology_data, merge_to_morphology_data, clone_morphology_data, create_mpi_morphology_type, morphology_area_attribute, morphology_attribute}, //18
  {"Max flux",sizeof(MorphologyData), new_morphology_data, init_morphology_data, delete_morphology_data, add_to_morphology_data, merge_morphology_data, merge_to_morphology_data, clone_morphology_data, create_mpi_morphology_type, morphology_area_attribute,  morphology_fmax_attribute}, //19
};
