...
void send_boundary(Boundary *b, int dest) {
  ...
  err = MPI_Send(b->attribute_idx, b->size_curr, MPI_INT64_T,  dest, tag, MPI_COMM_WORLD);
  ...
  err = MPI_Send(b->store->data, b->store->item_curr, mpi_attribute_type,  dest, tag, MPI_COMM_WORLD);
  check_mpi_error(err, 306);

} /* send_boundary */


Boundary *receive_boundary(int src, int size_item) {  
  /* Initialize MPI */
  ...
  b->store    = calloc(1, sizeof(AuxDataStore)); check_alloc(b->store, 301);
  ...
  b->attribute_idx  = malloc(recv_size * sizeof(idx));           check_alloc(b->attribute_idx, 304);
  ...
  err = MPI_Recv(b->attribute_idx, recv_size, MPI_INT64_T,  src, tag, MPI_COMM_WORLD, &status);
  ...
  b->size_curr = b->size_init = b->size_alloc = recv_size;

  /* Receive attribute */
  
  MPI_Probe(src, tag, MPI_COMM_WORLD, &status);
  MPI_Get_count(&status, mpi_attribute_type, &recv_size);

  init_aux_data_store(b->store, size_item, recv_size);
  err = MPI_Recv(b->store->data, recv_size, mpi_attribute_type, src, tag, MPI_COMM_WORLD, &status);
  check_mpi_error(err, 313);
  b->store->item_curr = recv_size;
...
Boundary *receive_updated_boundary(Boundary *b, int src) {
  ...
  if ((ulong) recv_size > b->size_curr) {
    ...
    b->attribute_idx    = realloc(b->attribute_idx,   recv_size * sizeof(idx));
    ...
    check_alloc(b->attribute_idx, 309);
  ...
  MPI_Probe(src, tag, MPI_COMM_WORLD, &status);
  MPI_Get_count(&status, mpi_attribute_type, &recv_size);
  b->store->size_alloc = recv_size;
  b->store->data       = realloc(b->store->data, b->store->size_alloc * b->store->size_item);
  check_alloc(b->store->data, 313);
  
  err = MPI_Recv(b->store->data, recv_size, mpi_attribute_type, src, tag, MPI_COMM_WORLD, &status);
  check_mpi_error(err, 323);
...
Boundary *receive_boundary_att(int src, int size_item) {  
  /* Initialize MPI */
  int 		tag = 1, err; // MPI Tag is ignored 
  MPI_Status 	status;
  int 		recv_size;
  ...
  b->store    = calloc(1, sizeof(AuxDataStore)); check_alloc(b->store, 319);
  ...
  b->attribute_idx  = malloc(recv_size * sizeof(idx));           check_alloc(b->attribute_idx, 322);
