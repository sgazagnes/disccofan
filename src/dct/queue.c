...
PrioQueue *create_prio_queue(ulong max_rank) {
  PrioQueue *queue = malloc(sizeof(PrioQueue));
  queue->m_num_levels = num_levels(max_rank);
  queue->m_levels = malloc(queue->m_num_levels* sizeof(ulong*)); check_alloc(queue->m_levels, 405);
  for (int i = queue->m_num_levels; i--;){
    max_rank >>= bits_per_word_log2();
    queue->m_levels[i] = calloc(max_rank+1, sizeof(ulong));
    check_alloc(queue->m_levels[i], 405);
  }
  queue->m_top = 0;
  return queue;
}
...
void prio_queue_remove(PrioQueue *q) {
  ...
  for (int i = q->m_num_levels; i--;){
	...
	{
          // given the word indexed by the current prefix, add the
          // index of the most significant bit to the back of the 
          // prefix
