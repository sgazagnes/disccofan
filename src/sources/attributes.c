...
//double data_properties.pixdim[3];
...
double area_attribute(void *areaattr){
   AreaData *areadata = areaattr;
   double area;
   area = data_properties.pixdim[0]*data_properties.pixdim[1]*data_properties.pixdim[2]*areadata->area;
   return(area);
} /* area_attribute */
...
void merge_extent_data(void *rectattr, void *childattr){
   ...
   ExtentData *childdata = childattr;
    // info("%ld, %ld", extentdata->maxX , childdata->maxX );
   extentdata->area +=  childdata->area;
   ...
   //  info("%ld, %ld", extentdata->minX);

  //  info("%ld", extentdata->minX);
} /* merge_extent_data */
...
void merge_to_extent_data( AuxDataStore *store, void **thisattr, void *rectattr, void *childattr){
  ...
  thisdata->area = extentdata->area + childdata->area;
...
double extent_area_attribute(void *rectattr) {
    ExtentData *extentdata = rectattr;
    return extentdata->area;
}

double extent_rectarea_attribute(void *rectattr) {
    ExtentData *extentdata = rectattr;

    // Calculate the volume based on the extents, casting to double
    double volume = (double)(extentdata->maxX - extentdata->minX) 
                  * (double)(extentdata->maxY - extentdata->minY)
                  * (double)(extentdata->maxZ - extentdata->minZ);

    // Scale the volume by the pixel dimensions
    volume *= data_properties.pixdim[0] * data_properties.pixdim[1] * data_properties.pixdim[2];
...
double extent_rectdiag_attribute(void *rectattr) {
    ...
    double minx = data_properties.pixdim[0] * (double)extentdata->minX;
    double miny = data_properties.pixdim[1] * (double)extentdata->minY;
    double minz = data_properties.pixdim[2] * (double)extentdata->minZ;
    double maxx = data_properties.pixdim[0] * (double)extentdata->maxX;
    double maxy = data_properties.pixdim[1] * (double)extentdata->maxY;
    double maxz = data_properties.pixdim[2] * (double)extentdata->maxZ;

    // Compute the square of the diagonal length
    double l = (maxx - minx) * (maxx - minx)
             + (maxy - miny) * (maxy - miny)
             + (maxz - minz) * (maxz - minz);
...
double extent_x_attribute(void *rectattr) {
    ...
    ExtentData *extentdata = rectattr;
    return data_properties.pixdim[0] * (double)(extentdata->maxX - extentdata->minX);
}

double extent_y_attribute(void *rectattr) {
    /* Computes the Y-extent */
    ExtentData *extentdata = rectattr;
    return data_properties.pixdim[1] * (double)(extentdata->maxY - extentdata->minY);
}

double extent_z_attribute(void *rectattr) {
    /* Computes the Z-extent */
    ExtentData *extentdata = rectattr;
    return data_properties.pixdim[2] * (double)(extentdata->maxZ - extentdata->minZ);
}
...
double mean_area_attribute(void *meanattr){
   MeanData *meandata = meanattr;
   return(data_properties.pixdim[0]*data_properties.pixdim[1]*data_properties.pixdim[2]*(double) meandata->area);
} /* mean_area */

double mean_x_attribute(void *meanattr){
   MeanData *meandata = meanattr;
   return( data_properties.pixdim[0]*(double)meandata->sumX / (double) meandata->area);
} /* mean_x_attribute */

double mean_y_attribute(void *meanattr){
   MeanData *meandata = meanattr;
   return(data_properties.pixdim[1]* (double)meandata->sumY / (double)  meandata->area);
} /* mean_y_attribute */

double mean_z_attribute(void *meanattr){
   MeanData *meandata = meanattr;
   return( data_properties.pixdim[2]* (double)meandata->sumZ / (double) meandata->area);
} /* mean_z_attribute */
...
void init_wmean_data(void *wmeanattr, bool init, ulong x, ulong y, ulong z, value gval){
  ...
  wmeandata->area = (ulong) init;
  wmeandata->sumGval = (double) init* (double) gval;
  wmeandata->sumXd = init ? (double) gval* (double) x : 0.;
  wmeandata->sumYd = init ? (double) gval* (double) y : 0.;
  wmeandata->sumZd = init ? (double) gval* (double) z : 0.;
}/* init_wmean_data*/
...
void merge_to_wmean_data( AuxDataStore *store, void **thisattr, void *wmeanattr, void *childattr){
   ...
   thisdata->area = wmeandata->area + childdata->area;
...
double wmean_area_attribute(void *wmeanattr){
   WMeanData *wmeandata = wmeanattr;
   return((double)wmeandata->area);
} /* mean_area */
...
double wmean_x_attribute(void *wmeanattr){
   WMeanData *wmeandata = wmeanattr;
   return((double) data_properties.pixdim[0]*wmeandata->sumXd / (double) wmeandata->sumGval);
} /* mean_x_attribute */

double wmean_y_attribute(void *wmeanattr){
   WMeanData *wmeandata = wmeanattr;
   return((double) data_properties.pixdim[1]*wmeandata->sumYd / (double) wmeandata->sumGval);
} /* mean_y_attribute */

double wmean_z_attribute(void *wmeanattr){
   WMeanData *wmeandata = wmeanattr;
   return((double) data_properties.pixdim[2]*wmeandata->sumZd / (double)  wmeandata->sumGval);
} /* mean_z_attribute */
...
void *inertia_arr(void *inertiaattr){
   ...
   inertia[0] = (double) data_properties.pixdim[0]*data_properties.pixdim[1]*data_properties.pixdim[2]*inertiadata->area;
   inertia[1] = data_properties.pixdim[0]*inertiadata->sumX;
   inertia[2] = data_properties.pixdim[1]*inertiadata->sumY;
   inertia[3] = data_properties.pixdim[2]*inertiadata->sumZ;
   inertia[4] = data_properties.pixdim[0]*data_properties.pixdim[0]*inertiadata->sumX2;
   inertia[5] = data_properties.pixdim[1]*data_properties.pixdim[1]*inertiadata->sumY2;
   inertia[6] = data_properties.pixdim[2]*data_properties.pixdim[2]*inertiadata->sumZ2;
   inertia[7] = data_properties.pixdim[0]*data_properties.pixdim[1]*inertiadata->sumXY;
   inertia[8] = data_properties.pixdim[1]*data_properties.pixdim[2]*inertiadata->sumYZ;
   inertia[9] = data_properties.pixdim[0]*data_properties.pixdim[2]*inertiadata->sumXZ;
   inertia[10] = inertiadata->sumval;
   inertia[11] = inertiadata->sumval2;
   inertia[12] = data_properties.pixdim[0]*inertiadata->sumXd;
   inertia[13] = data_properties.pixdim[1]*inertiadata->sumYd;
   inertia[14] = data_properties.pixdim[2]*inertiadata->sumZd;
   return(inertia);
} /* inertia_array */
...
void *inertia_attribute_arr(void *inertiaattr){
  ...
  attrArr[6] = inertia[12]/inertia[10]; // Weighted Mean X
  attrArr[7] = inertia[13]/inertia[10]; // Weighted Mean Y
  attrArr[8] = inertia[14]/inertia[10]; // Weighted Mean Z
  
  double corrVal    = inertia[0]/12;
  
  if(data_properties.dims_process[2]==1){		       
     double matrix[3] = { inertia[4] - inertia[1]*inertia[1]/inertia[0] +  data_properties.pixdim[0]*corrVal,
       inertia[5] - inertia[2]*inertia[2]/inertia[0] +  data_properties.pixdim[1]*corrVal,
     ...
     double matrix[6] = { inertia[4] - inertia[1]*inertia[1]/inertia[0] +  data_properties.pixdim[0]*corrVal,
       inertia[5] - inertia[2]*inertia[2]/inertia[0] +  data_properties.pixdim[1]*corrVal,
       inertia[6] - inertia[3]*inertia[3]/inertia[0] +  data_properties.pixdim[2]*corrVal,
       inertia[7] - inertia[1]*inertia[2]/inertia[0],
...
double inertia_area_attribute(void *inertiaattr){
   InertiaData *inertiadata = inertiaattr;
   double area = (double) data_properties.pixdim[0]*data_properties.pixdim[1]*data_properties.pixdim[2]*inertiadata->area;
   return(area);
...
double  inertia_trace_attribute(void *inertiaattr){
  //   InertiaData *inertiadata = inertiaattr;
   ...
   double trace = 0;
   if(inertia[3] == 0){
     double matrix[2] = { inertia[4] - inertia[1]*inertia[1]/inertia[0] + data_properties.pixdim[0]*corrVal,
       inertia[5] - inertia[2]*inertia[2]/inertia[0] + data_properties.pixdim[1]*corrVal };
...
double inertia_totalflux_attribute(void *inertiaattr){
   double *inertia  = inertia_arr(inertiaattr);
   return((double) inertia[10]);
} /* mean_area */
...
void init_attrib_array(void *attribute, value *gvals, ulong *dims, ulong lwb, ulong upb, ulong size_att){
  ulong width  = dims[0];
  ulong height = dims[1];
  ulong depth  = dims[2];
  ...
  for (ulong i =lwb; i < upb; i++){
    ...
    // if(rank()==0)
    //   printf("%ld, %ld, %lf, %d\n", x, y, gvals[i], init);
    // debug("%ld, %ld, %ld",x + attr_off[0], y + attr_off[1], z+ attr_off[2] );
    init_aux_data((attribute + i*size_att), !init, x + data_properties.offsets[0], y + data_properties.offsets[1], z+ data_properties.offsets[2], gvals[i]);
  }
}
...
{
 {"Area", "area", sizeof(AreaData), 0, new_area_data, init_area_data, delete_area_data, add_to_area_data, merge_area_data, merge_to_area_data, clone_area_data, create_mpi_area_type, area_attribute, area_attribute}, //0
 
 {"Area of minimal enclosing rectangle", "rectarea", sizeof(ExtentData),  1, new_extent_data, init_extent_data, delete_extent_data, add_to_extent_data, merge_extent_data, merge_to_extent_data, clone_extent_data, create_mpi_extent_type, extent_area_attribute, extent_rectarea_attribute}, //1
 {"Square of diagonal of minimal enclosing rectangle", "rectdiag", sizeof(ExtentData), 1, new_extent_data, init_extent_data, delete_extent_data, add_to_extent_data, merge_extent_data, merge_to_extent_data, clone_extent_data, create_mpi_extent_type, extent_area_attribute, extent_rectdiag_attribute},//2
 {"X-extent", "xext", sizeof(ExtentData), 1, new_extent_data, init_extent_data, delete_extent_data, add_to_extent_data, merge_extent_data, merge_to_extent_data, clone_extent_data, create_mpi_extent_type,extent_area_attribute, extent_x_attribute},//3
 {"Y-extent", "yext", sizeof(ExtentData), 1, new_extent_data, init_extent_data, delete_extent_data, add_to_extent_data, merge_extent_data, merge_to_extent_data, clone_extent_data, create_mpi_extent_type,extent_area_attribute, extent_y_attribute},//4
 {"Z-extent", "zext", sizeof(ExtentData), 1, new_extent_data, init_extent_data, delete_extent_data, add_to_extent_data, merge_extent_data, merge_to_extent_data, clone_extent_data, create_mpi_extent_type,extent_area_attribute, extent_z_attribute},//5
 
  {"Mean X position", "xmean", sizeof(MeanData), 2, new_mean_data, init_mean_data, delete_mean_data, add_to_mean_data, merge_mean_data, merge_to_mean_data, clone_mean_data,  create_mpi_mean_type, mean_area_attribute, mean_x_attribute},//6
 {"Mean Y position", "ymean", sizeof(MeanData), 2, new_mean_data, init_mean_data, delete_mean_data, add_to_mean_data, merge_mean_data, merge_to_mean_data, clone_mean_data,  create_mpi_mean_type, mean_area_attribute, mean_y_attribute},//7
 {"Mean Z position", "zmean", sizeof(MeanData), 2, new_mean_data, init_mean_data, delete_mean_data, add_to_mean_data, merge_mean_data, merge_to_mean_data, clone_mean_data,  create_mpi_mean_type, mean_area_attribute, mean_z_attribute},//8

 {"Weighted Mean X position", "xwmean", sizeof(WMeanData), 3, new_wmean_data, init_wmean_data, delete_wmean_data, add_to_wmean_data, merge_wmean_data, merge_to_wmean_data, clone_wmean_data,  create_mpi_wmean_type, wmean_area_attribute, wmean_x_attribute},//9
 {"Weighted Mean Y position", "ywmean", sizeof(WMeanData), 3, new_wmean_data, init_wmean_data, delete_wmean_data, add_to_wmean_data, merge_wmean_data, merge_to_wmean_data, clone_wmean_data,  create_mpi_wmean_type, wmean_area_attribute, wmean_y_attribute},//10
 {"Weighted Mean Z position", "zwmean", sizeof(WMeanData), 3, new_wmean_data, init_wmean_data, delete_wmean_data, add_to_wmean_data, merge_wmean_data, merge_to_wmean_data, clone_wmean_data,  create_mpi_wmean_type, wmean_area_attribute, wmean_z_attribute},//11
  {"Total flux", "flux", sizeof(WMeanData), 3, new_wmean_data, init_wmean_data, delete_wmean_data, add_to_wmean_data, merge_wmean_data, merge_to_wmean_data, clone_wmean_data,  create_mpi_wmean_type, wmean_area_attribute, wmean_totalflux_attribute},//12
 
 {"Trace of Inertia Matrice", "inertiatrace", sizeof(InertiaData), 4, new_inertia_data, init_inertia_data, delete_inertia_data, add_to_inertia_data, merge_inertia_data, merge_to_inertia_data, clone_inertia_data, create_mpi_inertia_type, inertia_area_attribute, inertia_trace_attribute}, //13
 {"Elongation", "elong", sizeof(InertiaData), 4, new_inertia_data, init_inertia_data, delete_inertia_data, add_to_inertia_data, merge_inertia_data, merge_to_inertia_data, clone_inertia_data, create_mpi_inertia_type, inertia_area_attribute, inertia_elon_attribute},//14
 {"Flatness", "flat", sizeof(InertiaData), 4, new_inertia_data, init_inertia_data, delete_inertia_data, add_to_inertia_data, merge_inertia_data, merge_to_inertia_data, clone_inertia_data, create_mpi_inertia_type, inertia_area_attribute, inertia_flat_attribute},//15
 {"Sparseness", "spars", sizeof(InertiaData), 4, new_inertia_data, init_inertia_data, delete_inertia_data, add_to_inertia_data, merge_inertia_data, merge_to_inertia_data, clone_inertia_data, create_mpi_inertia_type, inertia_area_attribute, inertia_spar_attribute},//16
 {"Non-compactnesss", "ncomp", sizeof(InertiaData), 4, new_inertia_data, init_inertia_data, delete_inertia_data, add_to_inertia_data, merge_inertia_data, merge_to_inertia_data, clone_inertia_data, create_mpi_inertia_type, inertia_area_attribute, inertia_ncom_attribute}, //17
};
