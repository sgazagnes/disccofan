...
int main(int argc, char** argv) {
  if(argc <= 1){
    ...
    if(!strcmp(argv[1], "create")){
      if (argc <= 9 || (atoi(argv[2]) < 0 || atoi(argv[2]) > 5) ) {
	...
	exit(0);
      ...
      else if(atoi(argv[2]) == 5)  im_out = create_gauss_field(dims[0], size);
    ...
    /**************************************************************************************/ 
      ...
      if (argc<=5) {
	printf("Function <bin_to_fits>: convert binary files to fits. Arguments: <input binary file name> <output name without type suffix> <output type suffix> <dimension of the sides>");
	exit(0);
      }

      ulong dims[3];
      if(argc == 6){
	dims[0] = dims[1] = dims[2] = atoi(argv[5]);
      }else{
	dims[0] = atoi(argv[5]);
	dims[1] = atoi(argv[6]);
	dims[2] = atoi(argv[7]);  
      ...
      /* mod_fread(temp, size * sizeof(float), 1, F);
      for(ulong i = 0; i<size; i++)
      im_out[i] = (double) temp[i]*/
    ...
    } else if( !strcmp(argv[1], "bintofitsdou") ){
      if (argc<=5) {
	printf("Function <bin_to_fits>: convert binary files to fits. Arguments: <input binary file name> <output name without type suffix> <output type suffix> <dimension of the sides>");
	exit(0);
      }
