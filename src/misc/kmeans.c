...
double calc_distance(int dim, double *p1, double *p2)
  {
    ...
    for (int ii = 0; ii < dim; ii++)
      distance_sq_sum += sqr((double) p1[ii] - p2[ii]);
    ...
    for (int ii = 0; ii < n; ii++)
      {
       ...
       // sum point coordinates for finding centroid
        for (int jj = 0; jj < dim; jj++)
          new_cluster_centroid[active_cluster*dim + jj] += (double) X[ii*dim + jj];
      }
    ...
    for (int ii = 0; ii < n; ii++)
      {
        ...
        for (int kk = 0; kk < dim; kk++)
          {
            double axis_dist = (double) X[ii*dim + kk] - cluster_centroid[cc*dim + kk]; 
...
void  perform_move(int dim, int n, int k, double *X, int *cluster_assignment, double *cluster_centroid, int *cluster_member_count, int move_point, int move_target_cluster)
  {
   ...
   // update centroid array
    for (int ii = 0; ii < dim; ii++)
      {
        cluster_centroid[cluster_old*dim + ii] -= ((double)X[move_point*dim + ii] - cluster_centroid[cluster_old*dim + ii]) / cluster_member_count[cluster_old];
        cluster_centroid[cluster_new*dim + ii] += ((double)X[move_point*dim + ii] - cluster_centroid[cluster_new*dim + ii]) / cluster_member_count[cluster_new];
      }
  }  
