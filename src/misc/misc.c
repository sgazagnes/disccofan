...
double *create_random(ulong size, int bitpix, int neg){
  ...
  for(ulong i = 0; i < size; i++){
    ...
    gvals_out[i] =  tt;//gsl_ran_flat( r, min_gval_out, max_gval_out);
  }
  return gvals_out;
}
...
double *create_bubbles_nov(ulong dim, ulong size,  float fract, int periodic){
  ...
  // double myRand = rand()/(1.0 + RAND_MAX); 
    int range = size + 1;
    // int myRand_scaled = myRand * range;
    double r = 3;//rand()%4 + 2;//round(randn(5, 3));
    // double r = 10;
    long x0 = 9;// myRand_scaled%dim;
    long z0 = 5;// myRand_scaled/(dim*dim);
    long y0 = 5;// myRand_scaled%(dim*dim)/dim;
    ...
    gvals_out[z0*dim*dim+(y0-2)*dim+x0-3]=255;
    gvals_out[z0*dim*dim+(y0-3)*dim+x0-3]=255;
...
double *create_gauss_field(ulong dim, ulong size){
  double *gvals_out = calloc(size, sizeof(double));
  //memset(gvals_out, 1, size * sizeof(double));
  srand(time(NULL));
  //  ulong npixels = (ulong) (fract * size);
